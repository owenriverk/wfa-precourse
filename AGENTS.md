...
6. **Scene art is layered SVG driven by the same state.** Visual changes must be loud (a mass appears or vanishes), not a 3-pixel mark. The Rider Down's patient figure is generated by `tools/patient_rig.py` — edit rig constants, never generated shapes.
